...
def build( 
    ...
    if clavicleJnt:
        ...
        # make again shoulder contraints
        '''
        ikBaseAttachGrp = transform.makeGroup( prefix = side + '_ikAttachRef', referenceObj = upperJnt, parentObj = limbData['ikBaseCtrl'].C ) 
        upperJntPointConst = mc.pointConstraint( upperFkCtrl.C, ikBaseAttachGrp, upperJnt )[0]
...
def _buildFkFoot( prefix, toeJnt, ankleJnt, limbIk1Ctrl, rigmodule, ctrlScale ):
...
def _buildIkFoot( prefix, doReverseFoot, limbIk1Ctrl, ikAttachGrp, ankleJnt, toeJnt, ctrlScale, limbIk, mainModuleGrp, heelLoc, tipLoc, outFootLoc, inFootLoc ):
